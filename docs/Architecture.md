...
flowchart TD
    ...
    BASE["ハンドラの基底クラス<br/>base_handler.py"]
...
| `download.py` | ダウンロードと試聴の 4 ハンドラ | `base_handler` `audition` `storage` `transpose` |
| `history.py` | 履歴の画面 | `base_handler` `conf` `rollbook` `storage` |
| `config_handler.py` | 機種設定の画面 | `base_handler` `conf` `rollbook` |
| `base_handler.py` | 全ハンドラの基底クラス（TODO-075） | `storage` |
...
かつては `handler1.py` に `StorganBaseHandler` もダウンロード処理も入っていて、`history.py` と
...
`AuditionMidi` と `DownloadTransposedMidi` は目的が違う（試聴とダウンロードする素材）
...
### 返すほうも基底クラスにまとめてある（TODO-095 / TODO-096）
...
位置、拡縮の位置合わせ）は [`webroot/CLAUDE.md`](../webroot/CLAUDE.md) にある
（TODO-090）。ここに書くのは Python 側とまたがるところだけ。
