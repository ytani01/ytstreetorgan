...
| [14](#14-バージョンを固定できない) | バージョンを固定できない | 運用 | 低 |
...
```toml
[tool.uv.sources]
ytmidilib = { git = "https://github.com/ytani01/ytmidilib.git" }
```

**既定ブランチの先頭を追う**ので、上流を直すたびに全利用者の挙動が変わる。
上の項目のいくつか（特に 4・7）は挙動が変わるものなので、
**タグを打ってほしい**（`v0.0.4` など）。そうすれば
`{ git = "...", tag = "v0.0.4" }` で固定でき、上げるタイミングを選べる。
...
- **1・2・3・9・10・11・13 は挙動を壊さない**（型と不具合の修正）。
  そのまま入れてよい
- **4・7 は出力や戻り値の形が変わる**。タグを打ってから入れてほしい（項目 14）
