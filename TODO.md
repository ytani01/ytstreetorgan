...
**番号は `TODO-060` から。** かつては A・B・C … の 1 文字だったが、
...
**残っている項目: 1 件。** これまでに 59 件を決着させた。

## TODO-060. ドキュメント docs/multi_agent_token_savings.md へレビューに基づく補足・提案の組み込み

- **内容**:
  - 「注意点・トレードオフ」節の「要約による情報の欠落」リスクに対し、調査エージェントに一次情報（コード構造・ファイルパス・行番号など）を出力させる対策を追記
  - 「実装方法」節の「具体的な委任方法」に、永続サブエージェント配置（`.agents/agents/`）を選択肢 B として追加・整理
