...
更新: 2026-08-19（TODO-103 を追加）

**残っている項目: TODO-103。** 新しく足すときは、 **完了済み** の上に節を作る（完了したら「完了済み」へ移す）。**番号は `TODO-104` から。**
...
- [ ] 機種セレクトの下の説明（`storgan.html:50-53`）を、リードの下へ移す
- [ ] フォームの下の保存と削除の案内（`storgan.html:82-85`）を、同じ場所へ移す
- [ ] 移した文章を、リードと合わせて 3〜4 行に収まる長さに削る
- [ ] ブラウザで、ドロップ欄が画面の下に押し出されていないか確かめる
- [ ] `pytest` / `ruff` / `mypy` を通す（文言を見るテストがあれば追従させる）

アップロード画面の説明が 4 か所に散っていて、読む順番が分かりにくい。
...
移す 2 つは、どちらも個々の操作の説明ではなく**この画面全体の振る舞い**の話。
今の位置だと、すぐ上の操作（機種を選ぶ・ファイルを選ぶ）の説明に見えて
誤解を招く。特に保存と削除の案内は、フォームの下にあるせいで
「アップロードした後の話」に読める。

**ドロップ欄の `.mid` / サイズの上限は動かさない。** あれは入力欄そのものの
仕様で、ファイルを選ぶ瞬間に目に入る位置にあることに意味がある。

懸念は高さ。リードの下に 3 行ぶら下がると、肝心のドロップ欄が下へ
押し出される。**移すだけでなく、文章を削ること。**

サブエージェントは編成しない（触るのは `storgan.html` と、必要なら
`my.css` だけ）。**Sonnet 5 / effort medium。**
