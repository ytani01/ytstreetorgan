...
- 一覧表で「試聴」の列は不要
- 「移調」を選択すると、SVG を生成するとともに、MIDI のロードも行い、下の
  プレーヤーは選択している移調候補の MIDI を鳴らせる状態にする

**決めごと: プレーヤーに読み込むところまでで、自動再生はしない。**
「移調」は `#transpose-form` の POST でページごと作り直す作りなので、
生成後の画面で、いま選ばれている移調量の音（`/audition/midi/…`）を
`<midi-player>` に読み込んでおく。再生は利用者が押す。自動再生は
ブラウザが止めることがあり、環境によって鳴ったり鳴らなかったりする。

### **TODO-069** 機種設定の音階マッピングについて

- 音名をドロップダウンで選択したら、音名リスト全体をソートする

**決めごと: 音名の昇順（低い音が上）に並べ替える。**
`'notes'` の並び順がそのままトラック番号なので、**穴の列の並びも変わる**
（表示だけ並べ替えて保存は入力順、という形にはしない。画面の行番号と
トラック番号が食い違うため）。

TODO-067 のあとに着手する。`base_note` を廃止すると並べ替えの基準が
