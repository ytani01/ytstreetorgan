...
更新: 2026-09-05（TODO-088 を追加）

- 新しく足すときは、 **完了済み** の上に節を作る（完了したら「完了済み」へ移す）。**番号は `TODO-089` から。**
...
- [ ] 1. `TODO.md` の骨格をグローバルの形に揃える（`# TODO` / `## 完了済み`、
      `=` 記法と `## == 着手前 / 検討中` を外し、冒頭を残件と決着件数にする）
- [ ] 2. `.claude/settings.json` の SessionStart フックを削除する
- [ ] 3. `CLAUDE.md` に「トークン消費量の集計」の節を足す

`~/.claude/CLAUDE.md` が 2026-09-05 に更新され、`archives/todo/` の表が
2 つに分かれた（担当ごとにモデル・effort・`output`・`cache_creation`・
料金の割合を書く）。集計の方法はプロジェクト側に書く決まりなので、
このリポジトリには記述が無い。突き合わせたところ、他にも 2 点ずれていた。

- **フックが二重**。グローバルの `~/.claude/hooks/todo-count.sh` が全
  プロジェクトで走るのに、`.claude/settings.json` にも同じものがある。
  しかも `/^### /` で数えていて、項目の見出しは `## TODO-NNN.`（h2）
  なので**常に 0 件**。TODO-082 で直したのはチェックボックスを数えて
  いた点で、見出しレベルは合っていなかった
- **`TODO.md` の骨格**が `# = TODO` / `## == 完了済み` で、グローバルの
  形と違う。グローバルのフックは `## 完了済み` / `## TODO-NNN.` を前提に
  するので、揃えないとフックを任せられない

集計は `~/bin/token-usage.py`（実体は `~/dot.files/token-usage.py`。
`~/dot.emacs.d` の TODO-021 で汎用化されたもの）をそのまま使う。移植は
要らない。**transcript は 2026-09-05 のぶんしか残っていない**ので、
TODO-087 以前は遡れない。

過去 87 件のアーカイブに表を足すことはしない（TODO-057 で「過去は
直さない」と決めてある）。

`verifier` を入れるのは、`CLAUDE.md` に書くコマンド例が**書いたとおりに
試せる手順**だから（グローバルの TODO-017）。フックを消したあとに件数が
出ることも確かめさせる。
