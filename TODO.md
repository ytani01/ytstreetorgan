...
更新: 2026-08-19（TODO-102 を追加）

**残っている項目: TODO-102。** 新しく足すときは、 **完了済み** の上に節を作る（完了したら「完了済み」へ移す）。**番号は `TODO-103` から。**

- **やらないと決めたものもある。** 目次で（対応しない）と付いたもののほか、TODO-029 のホイール拡縮、TODO-031 の設定キャッシュなど、項目の中の一部だけ見送ったものもある。蒸し返す前に記録を読むこと。

---

## == TODO-102. アップロード画面に、ファイルがサーバーに残ることと削除できることを書く

- [ ] `storgan.html` のファイル選択の節に 1 文足す
- [ ] ブラウザで見た目を確かめる
- [ ] `pytest` / `ruff` / `mypy` を通す

アップロードした MIDI と生成した SVG は `webroot/midi/` と `webroot/svg/` に
残るが、画面のどこにも書いていない。削除は履歴画面からできるものの、そこへ
行けると分かる手掛かりが無い。
...
「履歴」は `{{urlprefix}}/history` へのリンクにする。置き場所は `<article>` の
外、`drop__status` の下（フォームの操作を邪魔しない位置）。`field__hint` と
同じ小さい字で 1 行に収め、画面の高さを増やさない。

テストを足すかは、着手時に `tests/browser/test_rollbook_page.py` の書き方を
見て判断する。

モデル / effort: テンプレート 1 か所と CSS の微調整だけなので、サブエージェントは
編成しない。Sonnet / effort 中。
