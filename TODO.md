...
- [ ] `base.html` のナビゲーションリンク「MIDIアプロード」を「MIDIアップロード」にタイポ修正する
- [ ] 意図的な webUI 表示変更（ナビゲーション文言、諸元表記、凡例・説明文のコメントアウト等）に合わせて失敗しているテスト（`tests/test_history.py`, `tests/test_rollbook_page_http.py`）を更新する
- [ ] すべてのテスト（`pytest`）を通過させる

背景と決めること:
直近のコミット（`b5e99971`）での webUI 表示調整により、「MIDIアプロード」というタイポおよび画面文言・レイアウトの変更が発生し、関連する 5 件のテストが失敗している。
タイポを修正しつつ、ユーザーの意図した表示変更に合わせてテストケースを更新する。
