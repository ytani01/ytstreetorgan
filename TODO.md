...
**残っている項目: TODO-112。** これまでに 111 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-113` から。**

**やらないと決めたものもある。** 目次で（対応しない）と付いたもののほか、
TODO-029 のホイール拡縮、TODO-031 の設定キャッシュなど、項目の中の一部だけ
見送ったものもある。蒸し返す前に記録を読むこと。
...
- [ ] `CLAUDE.md` の「1 か所の修正や、文書だけの直しでは編成しない」を、
      文書だけの項目に限る例外として書き直す（コードやファイルを変える
      項目では、規模によらず確認の担当を分ける）
- [ ] `CLAUDE.md` に、確認・レビューは `~/.claude/agents/` の `verifier` /
      `reviewer` に分けると明記する（このリポジトリの 5 体は実装と調査の
      担当で、確認の担当ではない）
- [ ] `.claude/agents/*.md` 5 体の `description` に「管理者から名指しされた
      ときだけ使う」を足す

`~/.claude/CLAUDE.md` が更新され、「コードやファイルを変える項目では、
規模によらず確認の担当を別のサブエージェントに分ける」が入った。
このリポジトリの `CLAUDE.md` には「1 か所の修正や、文書だけの直しでは
編成しない」とあり、正面から食い違っている。

- 5 体（`core` / `web` / `tests` / `docs` / `survey`）の構成は変えない。
  ドメインで切ってあるのは TODO-088 の判断で、実績もある。グローバルの
  `implementer` / `reviewer` / `verifier` / `wording` と併存させ、
  確認・レビューだけそちらに寄せる
- `show-image` skill、Bash の hook、codegraph、`token-usage.py` は
  このリポジトリ側の対応不要（グローバルで完結、または追従済み）
- 変えるのは文書と定義ファイルだけなので、確認の担当は分けない
  （グローバルの例外にあたる）。ただし `.claude/agents/` は Claude Code の
  起動時にしか読まれないため、**反映には再起動が要る**（実行するのは利用者）
...
- [**TODO-111.** まとめ直した処理（TODO-097〜099）が文書に書かれていない](archives/todo/TODO-111.%20まとめ直した処理が文書に書かれていない.md)
- [**TODO-110.** 利用者向けの文書が、アップロード画面の変更に追従していない](archives/todo/TODO-110.%20利用者向けの文書が、アップロード画面の変更に追従していない.md)
- [**TODO-109.** docstring と、用語・実装の食い違いを直す](archives/todo/TODO-109.%20docstring%20と、用語・実装の食い違いを直す.md)
- [**TODO-108.** README を手入れする](archives/todo/TODO-108.%20README%20を手入れする.md)
- [**TODO-107.** アーキテクチャの説明を `docs/Architecture.md` に切り出す](archives/todo/TODO-107.%20アーキテクチャの説明を%20Architecture.md%20に切り出す.md)
- [**TODO-106.** 利用者向けの使い方を `docs/User.md` に書く](archives/todo/TODO-106.%20利用者向けの使い方を%20User.md%20に書く.md)
- [**TODO-105.** TODO 運用をグローバルの更新に合わせる](archives/todo/TODO-105.%20TODO%20運用をグローバルの更新に合わせる.md)
- [**TODO-104.** アーカイブに、実施したモデルと effort を残す](archives/todo/TODO-104.%20アーカイブに、実施したモデルと%20effort%20を残す.md)
- [**TODO-103.** アップロード画面の説明を、タイトルの下にまとめる](archives/todo/TODO-103.%20アップロード画面の説明を、タイトルの下にまとめる.md)
- [**TODO-102.** アップロード画面での保存と削除の案内を書く](archives/todo/TODO-102.%20アップロード画面での保存と削除の案内を書く.md)
- [**TODO-101.** 文書とコメントを「日本語の書き方」に合わせる](archives/todo/TODO-101.%20文書とコメントを「日本語の書き方」に合わせる.md)
- [**TODO-100.** 「土台」「持ち帰り」を分かりやすい語に直す](archives/todo/TODO-100.%20「土台」「持ち帰り」を分かりやすい語に直す.md)
- [**TODO-099.** JS の通信と小物が 3 ファイルに写してある](archives/todo/TODO-099.%20JS%20の通信と小物が%203%20ファイルに写してある.md)
- [**TODO-098.** `MidiApp.main()` の `parse_only` の分岐が 4 か所に散っている](archives/todo/TODO-098.%20MidiApp.main%28%29%20の%20parse_only%20の分岐が%204%20か所に散っている.md)
- [**TODO-097.** `Handler1.post()` が 88 行で、解析の失敗の扱いが二重](archives/todo/TODO-097.%20Handler1.post%28%29%20が%2088%20行で、解析の失敗の扱いが二重.md)
- [**TODO-096.** ダウンロード系のヘッダ組み立てが 3 か所に写してある](archives/todo/TODO-096.%20ダウンロード系のヘッダ組み立てが%203%20か所に写してある.md)
- [**TODO-095.** ハンドラの JSON 応答が 2 か所に写してある](archives/todo/TODO-095.%20ハンドラの%20JSON%20応答が%202%20か所に写してある.md)
- [**TODO-094.** `src/ytstreetorgan/mylog-new.py` が残っている](archives/todo/TODO-094.%20src／ytstreetorgan／mylog-new.py%20が残っている.md)
...
- [**TODO-089.** 起動時の残件表示が、今の項目見出しに合っていない](archives/todo/TODO-089.%20起動時の残件表示が、今の項目見出しに合っていない.md)
- [**TODO-088.** 使い回せるサブエージェントの定義を `.claude/agents/` に置く](archives/todo/TODO-088.%20使い回せるサブエージェントの定義を置く.md)
- [**TODO-087.** ドキュメントと実装の食い違いを直す](archives/todo/TODO-087.%20ドキュメントと実装の食い違いを直す.md)
- [**TODO-086.** `mylog.py` を差し替え、呼び出し側を `getLogger` に移行する](archives/todo/TODO-086.%20mylog.py%20を差し替え、呼び出し側を%20getLogger%20に移行する.md)
...
- [**TODO-064.** 機種設定で、音名(国際標準)でドロップダウンメニューで入力するように変更](archives/todo/TODO-064.%20機種設定で、音名%28国際標準%29でドロップダウンメニューで入力するように変更.md)
