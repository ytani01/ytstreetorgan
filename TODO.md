...
**残っている項目: TODO-113。** これまでに 112 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-114` から。**
...
- [ ] `.appbar__inner` の padding を `.2rem 1rem` → `.15rem 1rem`
- [ ] `.appbar nav a` の padding を `.35rem .8rem` → `.2rem .8rem`
- [ ] ブラウザで 3 画面（アップロード / 履歴 / 機種設定）の見た目を確かめる

ヘッダーの高さが約 37px あり、画面の上を余分に使っている。実質の高さを
決めているのは nav リンクの上下 padding なので、そこと `.appbar__inner` の
padding を詰めて約 31px にする。ロゴ SVG と文字の大きさは変えない
（印象を変えずに数 px 詰めるのが狙い）。
...
- [**TODO-112.** サブエージェントの運用を、更新後のグローバル規則に合わせる](archives/todo/TODO-112.%20サブエージェントの運用を、更新後のグローバル規則に合わせる.md)
