...
更新: 2026-08-19（TODO-091 を追加）

- 新しく足すときは、 **完了済み** の上に節を作る（完了したら「完了済み」へ移す）。**番号は `TODO-092` から。**
...
- [ ] `CLAUDE.md` に「サブエージェント」の節を足す
- [ ] 役割の一覧は書かないこと（各定義の `description:` にある）を確かめる

モデル / effort: Sonnet 5 / low

TODO-088 で `.claude/agents/` に 5 体（`core` / `web` / `tests` / `docs` /
`survey`）を置いたが、**どういうときに誰へ投げるかがどこにも書いていない**。
グローバルの `~/.claude/CLAUDE.md` には「規模の大きい項目は分担する」という
原則があるだけで、このリポジトリでの目安が無い。

**狙いはトークンを減らすこと。** 書く内容:

- 読むだけの調査は `survey` に投げる。grep の出力が大量に出ても、統括の
  文脈には結論だけが返る（ここが一番効く）
- 1 か所の修正・文書だけの直しでは編成しない（起動のたびに文脈を読み直す
  ので、分担で減らせるぶんより高くつく）
- 触ってよいファイルが重ならない担当は並列で動かす。テストを回す担当は
  最後の 1 体にする（TODO-067 の実績）
- 続きは `SendMessage` で同じ担当に頼む。`Agent` で呼び直すと文脈を
  読み直すところからになる
- モデルと effort は定義のままにする。上書きするのは、設計の判断が要る
  画面で `web` を Opus にするときだけ
- 統括が既に調べたこと（パス・関数名・決めたこと）は指示に書く。同じ調査を
  担当にやり直させない

**役割の表は書かない。** TODO-088 で決めたとおり、一覧を二重に持つと
実態と食い違う（TODO-061 で作った一覧が食い違い、TODO-080 で削除になった）。
`CLAUDE.md` はすべてのセッションで読み込まれるので、節そのものも短くする
（TODO-090 と同じ理由）。
