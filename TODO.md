...
更新: 2026-08-19（TODO-092〜094 を追加）
...
- [ ] `.claude/settings.json` の awk のパターンを直す
- [ ] 未完の項目がある状態と、無い状態の両方で出力を確かめる

モデル / effort: Sonnet 5 / low

TODO-089 で `/^### /` から `/^## TODO-/` に変えたが、`TODO.md` の項目は
`## == 着手前 / 検討中` の下にぶら下がるので `### TODO-091.` と 3 個で書く。
`^## TODO-` は 3 文字目が `#` なので当たらない。実際に、未完 1 件だった
`d1caa05` の `TODO.md` に `settings.json` のコマンドをそのまま当てると、
**何も出ずに終了する**（TODO-089 で直したはずの状態に戻っている）。

`/^#+ +TODO-/` にすれば `##` でも `###` でも数えられる。
`## == 着手前 / 検討中` は `TODO-` を含まないので当たらず、
`/^## .*完了済み/{exit}` の打ち切りもそのままでよい。
