...
- [ ] 代表的な 12 本から、実用的なカバレッジまで広げる

土台は `tests/browser/` に用意済み（`conftest.py` の `live_server` fixture、
`pytest -m browser` で実行）。現状は 12 本:

- `test_rollbook_page.py` 4 本 — アップロード → プレビュー、ビューアの初期位置と
  倍率ボタン、静的ファイルの読み込み
- `test_config_editor.py` 4 本 — 機種選択でフォームが埋まる、トラック行の
  追加・削除と番号の振り直し、追加ダイアログの引き継ぎ、保存の往復
- `test_model_persistence.py` 4 本 — 選択中の機種を画面間で引き継ぐ（`ModelStore`）

未着手の領域:

- 設定エディタ: 機種の追加を**実際に確定する**ところ（`btn-confirm-add-model`。
  いまはダイアログが引き継ぐところまでしか見ていない）、機種の削除。
  削除は `confirm()` を使っているので、Playwright 側で dialog を捌く必要がある
- 入力値の検証: 必須項目を空にした場合、数値欄に不正な値を入れた場合に
  どうなるか（サーバー側の 400 がユーザーにどう見えるか）。
  トラックの `offset` に数値でない値を入れた場合も含む
