...
**番号は `TODO-036` から。** かつては A・B・C … の 1 文字だったが、
...
- [ ] `viewer.js` の `Z_MAX` を 5.0 → 10.0 にする
...
- スライダの刻みは対数なので、目盛の数（0〜100）は変えなくてよい。
  上限が伸びた分だけ、同じつまみの位置がやや高い倍率を指すようになる
- 既定の表示は「高さ合わせ」のままで、上限の変更に影響されない
