...
**残っている項目: TODO-089、TODO-090、TODO-091。** これまでに 88 件を決着させた。
...
## TODO-089. 利用者向けの使い方を `docs/User.md` に書く
...
- [ ] `docs/User.md` を新しく作る
- [ ] Web UI の一連の流れを書く（アップロード → 移調候補の選択 → 試聴 →
      SVG の持ち帰り、履歴、機種設定）
- [ ] CLI の各サブコマンド（`rollbook` / `parse` / `play` / `webapp`）の
      使い方を書く
- [ ] README から `docs/User.md` へリンクする

利用者向けの説明が README の CLI 3 行しか無い。Web UI は画面が
5 つ（生成・履歴・機種設定と、持ち帰り・試聴）あるのに、どこにも
説明が無く、初めて触る人が移調候補の意味も試聴の位置付けも分からない。

用語は `CLAUDE.md`「画面に出す用語」の表に合わせる（トラック / 音名 /
音階 / 移調 / 音の長さ / 試聴）。**「調」「オクターブ」は画面に出さない**
ので、Web UI の説明では使わない。

書いたとおりに試せるコマンド例と画面の手順を含むので、再現の確認は
`verifier` に分ける（TODO-017）。

---

## TODO-090. アーキテクチャの説明を `docs/Architecture.md` に切り出す

|      | main | 担当 |
|------|------|------|
| 見込み | Opus 5 / effort high | verifier |

- [ ] `docs/Architecture.md` を新しく作る
- [ ] `CLAUDE.md` のアーキテクチャ節を人間の開発者向けに移す
- [ ] `CLAUDE.md` 側を要点とリンクに絞る
- [ ] `docs/Developer.md` / `docs/tech-stack.md` との相互のリンクを揃える

レイヤー分離、設定ファイルの形、SVG 座標系、穴の扱い、Web 層、
ビューアの作りは、人間の開発者にも要る話なのに `CLAUDE.md` にしか無い。

**同じ話を 2 箇所に残さない**（残すと片方だけ古くなる）。

決めること: どこまで移すか。「画面に出す用語」は Claude への指示の色が
濃いので `CLAUDE.md` に残す見込み。着手時に、節ごとの振り分け案を出して
利用者に確認する。

---

## TODO-091. README を手入れする
...
- [ ] 「【未完成】」の扱いを決めて直す
- [ ] 末尾「A. 手回しオルガン用ロール・ブック」の節を整理する
- [ ] `docs/` の各文書へのリンクを揃える

A 節はメモのまま書き捨ててある。`1秒 = 約5cm` は機種設定の
`'mm_per_sec'`（既定 50.0）の値なので、そう書く。「34音、低音部で、
一部半音がない」も設定次第で変わるので、同梱のテンプレートの話だと
分かるように書く。

**TODO-089 と TODO-090 のあとに着手する**（リンク先ができてから）。
...
- [**TODO-088.** TODO 運用をグローバルの更新に合わせる](archives/todo/TODO-088.%20TODO%20運用をグローバルの更新に合わせる.md)
