...
F（ブラウザテスト）で見つけた。**どちらも現状をテストで固定してある**ので、
直したら `tests/browser/test_rollbook_page.py` の該当テストも直すこと。

- [ ] MIDI でないファイル → 素の 500 ページ
- [ ] サイズ上限超え → 真っ白なページ

`Handler1.post()` が `RollBook.parse_to_file()` の例外を捕まえていないため、
壊れたファイルや MIDI でないファイルを送ると tornado 既定の
「500: Internal Server Error」に置き換わる。**画面そのものが失われる**ので、
利用者は選び直すこともできず、ブラウザの戻るしかない。

上限超えのほうは tornado が `max_buffer_size` で本文を読まずに接続を切るため、
ブラウザには何も表示されない（こちらはハンドラまで来ないので、直すなら
クライアント側で `input.files[0].size` を見て送信前に止めるのが素直）。

方針:

1. `post()` を `try` で囲み、失敗したら `_render(msg=...)` でファイル選択の
   画面に戻す。`msg` は既に「Please select a MIDI file」を出すのに使っていて、
   `.drop__status` に表示される経路があるので、そこに理由を出せばよい
2. 上限は `size_limit` をテンプレートに渡してあるので、JS で送信前に比較する
...
1. **J（アップロードの失敗表示）** — 小さく、利用者に直接見える不具合。
   直したときの確認はテストが受け持つ。
2. **I（live reload）** — 小さく、開発中ずっと効く。
