...
決めて決着）。**W は着手中**（W-1 のうち 4 と 27 が残り）。

---

## W. コード全体の見直し（リファクタリング）

`src/` を一通り読んだ棚卸し。**W-1 は 4 と、あとで見つかった 27 を除いて片付いた。**
W-2 以降は未着手。
...
- [ ] 27. **`tests/test_webapp_async.py` はまだ実物の `webroot/` を使っている。**
  ...
  `webroot/svg/dummy.mid.svg` を実際に書いて `tearDown` で消している。
  途中で落ちれば消し残る。→ `WebAppTestCase` に載せ替える
