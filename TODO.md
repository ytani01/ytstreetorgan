...
- [ ] 件数を項目見出し（`### `）の数で数える
- [ ] 打ち切り条件を今の `## == 完了済み` に合わせる
- [ ] 項目名は出さず、件数だけにする

`.claude/settings.json` の SessionStart フックが `TODO.md` を awk で読んで残件を表示している。`TODO.md` の見出しが `## TODO-NNN. 見出し` から `### **TODO-NNN** 見出し` に変わったのに、フックが追随していない。

- 件数を `- [ ]` の数で数えている（項目 4 件に対して 11 と出る）
- 打ち切り条件 `/^## 完了済み/` が `## == 完了済み` に一致せず、止まらない。項目名として節見出しを拾ってしまう
...
**項目名は出さない。** 4 件ぶん並べると 1 行が長くなるうえ、詳しくは `TODO.md` を読めば分かる。
