...
**残っている項目: TODO-110、TODO-111。** これまでに 109 件を決着させた。
...
- [ ] `docs/images/web-upload.png` を今の画面で撮り直す
- [ ] `docs/User.md` のアップロード画面の説明を、今の文言に合わせる
- [ ] 残りの 4 枚（`web-result` / `web-transpose` / `web-history` / `web-config`）が
      今の画面と合っているか確かめ、ずれていれば撮り直す
...
まとめている（`webroot/templates/storgan.html`）。少なくとも
`web-upload.png` は今の画面と違う。

撮影用に `docs/images/make_shots.py` が入っているので、まずこれがそのまま
使えるかを確かめる。
...
- [ ] `webroot/CLAUDE.md` に `webroot/static/js/api.js` を書く（JS の通信は
      ここに集約してあること。TODO-099）
- [ ] `Handler1.post()` の分割（TODO-097）と、`MidiApp.main()` の `parse_only`
      の集約（TODO-098）を `docs/Architecture.md` に書くかどうかを決める

マージのときの調査で見つけた。食い違いではなく、もともとどこにも書いて
いない（分岐前の `CLAUDE.md` にも無かった）。

- `api.js` は書いたほうがよい。`history.js` と `config_editor.js` から
  通信を集めた先なので、知らないと同じものをまた書く
- `Handler1.post()` と `parse_only` は、モジュールの中の分け方であって
  モジュールをまたぐ決めごとではない。`docs/Architecture.md` の粒度に
  照らすと、書かないほうが揃う

**2 つ目は着手のときに決める。**「書かない」で決着してよく、そのときは
理由を archives に残す。
