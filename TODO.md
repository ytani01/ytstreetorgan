...
**番号は `TODO-058` から。** かつては A・B・C … の 1 文字だったが、
...
- [ ] `pyproject.toml` のタグを `0.1.1` → `0.2.1` にして `uv sync` する
- [ ] `docs/tech-stack.md` のロギングの記述を直す
- [ ] `Parser(debug=...)` / `Player(debug=...)` の扱いを決める
- [ ] `pytest` / `ruff` / `mypy` と、CLI の実機確認

0.2.0 で**ロギングが標準 `logging` から loguru（`mylog.py`）へ変わった**
（上流の TODO-007。破壊的変更）。`my_logger.py` は無くなったが、
`ytmidilib/__init__.py` のエクスポート（`Parser` / `Player` / `NoteInfo` /
`transpose_file`）は変わっていないので、こちらの import はそのままでよい。

影響するのは 2 点。

- `docs/tech-stack.md` の「`ytmidilib` は標準 `logging` を使い、ハンドラを
  付けない」「**向こうのログはどこにも出ない**」が**嘘になる**。
  こちらの `loggerInit()` が張ったシンクへ向こうのログも流れるので、
  `-d` を付けると `ytmidilib` の DEBUG も出る
- `Parser(debug=)` / `Player(debug=)` は引数としては残っている（上流が
  互換のために残した）が、**水準に影響しない**。`MidiApp` の `debug` は
  この 2 つへ渡すためだけにあるので、持っている意味が無くなる。
  `RollBookApp` が同じ理由で `debug` を外した前例がある（`apps.py` の
  コメント）

0.2.1 には他に、未使用の依存 `sounddevice` の削除と、リファレンス
マニュアル（`docs/REFERENCE.md`）の追加が入っている。
