...
- [ ] `archives/todo/` の TODO-001〜087 と、`archives/agents/TODO-063` /
      `TODO-067` の定義を読み、繰り返し出てくる役割を洗い出す
- [ ] 役割ごとに `.claude/agents/<name>.md` を作る
- [ ] 各定義に次の 5 つを書く
  - モデル（`model:`）
  - effort（`effort:`）
  - 触ってよいファイルと、触ってはいけないもの
  - ツール呼び出しの上限回数
  - 出力を作る直前に、`CLAUDE.md` など関わる決めごとのファイルを
    読み直すこと

`.claude/agents/` は今は無い。`archives/agents/` にある 2 組は、
その項目のためだけに作った記録で、使い回せる形になっていない。

決めること:

- どの役割をいくつ作るか（`archives/agents/` の 2 組は `core` / `web` /
  `tests` / `docs` に近い分け方をしている）
- 各定義が読み直すファイルはどれか（`CLAUDE.md`、`webroot/CLAUDE.md`、
  `tests/CLAUDE.md`、`docs/Developer.md`）
- ツール呼び出しの上限を役割ごとにいくつにするか
- 完了後、これらは `.claude/agents/` に残す（現行の設定なので
  `archives/agents/` へは移さない）でよいか

モデル / effort: Opus / medium（既存の TODO とエージェント定義を読み通し、
役割の切り分けを決める判断が要る）。
