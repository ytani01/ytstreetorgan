...
更新: 2026-08-13（TODO-084 を追加）

- 新しく足すときは、 **完了済み** の上に節を作る（完了したら「完了済み」へ移す）。**番号は `TODO-085` から。**
...
モデル: Sonnet / effort medium（テンプレートと CSS の手直し。判断が
要るのは 1 と 9 だけ）。

811372d「画面構成をシンプルにした。」のあと、**文言と実態が食い違う所**と
**中身だけ消えて残った空要素**がある。テストは通常 297 / ブラウザ 49 とも
通っているので、テストでは気付けない。

**811372d の狙いは次の 3 つ。直すときに元へ戻さないこと。**

- **説明を削ぎ落とす。** くどかった。人間が見れば分かることは書かない
- **散らばっていたボタンをまとめる**
- **ページの高さを縮める**（スクロールしなくて済むように）

この狙いに沿っているので**戻さないと決めたもの**（当初 2 / 7 / 8 / 9 に
挙げていた）:

- 「鳴るのは**この機種で実際に鳴る音だけ**」の一文 — 書かなくても分かる
- 音名の丸括弧が MIDI ノート番号だという説明 — 数字を見れば分かる
- ブリッジの説明と、独立した「ブリッジ」の節 — 無駄に高さを取っていた。
  「基本寸法」に統合したままでよい
- 音階マッピングの表の高さ — いまのままでよい

**1 は直し方が複数あるので、着手前に方針を決める。**

### ロールブックの画面（`webroot/templates/storgan.html`）

- [ ] 1. **表が無いのに「下の表」と書いてある。** 試聴プレーヤーが
  `<section class="panel">` の外へ出たので、`show_transpose_table` が
  false のとき（移調しても改善しないとき）にも出るようになった。
  文面は「下の表で移調を選ぶと」のまま。`tests/data/in-scale.mid` で再現。
  **プレーヤーが出ること自体は改善**なので、文面を条件で分けるか、
  表があるときだけ出すかを決める
- [ ] 2. 改行の入れ方で「移調しない場合␣の音を」と余分な空白が入る

### 機種設定の画面（`webroot/templates/config_editor.html`）

- [ ] 3. `<p class="page-lead">` が中身だけ空で残っている（余白だけ出る）。
  タグごと削除する
- [ ] 4. `<thead>` が空。**見出しは戻さず**、空の `<thead>` を削除する
- [ ] 5. 表の下の `<div class="panel__body" style="border-top:…">` が
  空のまま残り、**罫線と余白だけ**が出る。タグごと削除する

### CSS（`webroot/static/css/my.css`）

- [ ] 6. `.savebar` / `.savebar__msg` / `.savebar button`（499-509 行）が
  未使用になった
- [ ] 7. 366 行のコメント「左カラム（基本寸法＋ブリッジ）」が実態と違う
