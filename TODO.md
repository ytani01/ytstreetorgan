...
更新: 2026-08-19（TODO-089 を立てる）

- 新しく足すときは、 **完了済み** の上に節を作る（完了したら「完了済み」へ移す）。**番号は `TODO-090` から。**
...
- [ ] `.claude/settings.json` の SessionStart フックの awk が数える行を、
      今の項目見出し（`## TODO-NNN. 見出し`）に合わせる
- [ ] 直したあと、合成した `TODO.md` で件数が出ることを確かめる
      （残件あり / 残件ゼロ / `TODO.md` が無い、の 3 通り）

いまの awk は `/^### /` を数える。

```awk
/^## .*完了済み/{exit} /^### /{n++} END{if(n>0)printf "TODO に残っているのは %d 件", n}
```

TODO-082 で直したときの見出しは `### **TODO-NNN** 見出し` だったが、
6656a03（2026-08-12）以降は `## TODO-NNN. 見出し`
（`~/.claude/CLAUDE.md` の骨格に合わせた形）になっている。
`### ` の行が無いので `n` が 0 のままになり、**残件があっても何も出ない**。
残件 1 件の `TODO.md` を合成してフックの awk をそのまま実行し、
出力が無いことを確認した。

打ち切り条件 `/^## .*完了済み/` は今の `## == 完了済み` に一致するので
そのままでよい。数える行を変えるとき、`## == 着手前 / 検討中` を
拾わないようにすること。

モデル / effort: Sonnet / low（awk 1 行の書き換えと、その確認だけ）。
