...
**番号は `TODO-038` から。** かつては A・B・C … の 1 文字だったが、
...
- [ ] `setZoom()` の位置合わせを、`scrollWidth` に対する比から
  「基準の点が SVG の右端から何 mm か」に変える

拡大すると、画面の中央に出ていた位置（`#pos-mm` の値）がずれる。
長いブックの中の一か所を拡大して見たいときに、見ていた場所を見失う。

原因は位置の覚え方。いまは `(scrollLeft + clientWidth / 2) / scrollWidth`
の比で覚えて、拡縮後に同じ比へ戻している。これがずれるのは 2 つ理由がある。
...
- 覚えるのは**ブック上の位置（mm）**にする。`update()` が `#pos-mm` を
  出すのと同じ、SVG の右端からの距離。これなら倍率と無関係に決まる
- 縦も同じように保つ。高さ合わせから拡大すると縦にもはみ出すため
- ホイール拡縮（Ctrl / ⌘ + ホイール）は**ポインタの位置**を基準にする。
  いまも比で渡しているので、渡し方を「画面上の x 座標」に変える
- 「高さ合わせ」「全体」は拡縮のあと右端へ寄せる。**この動きは変えない**
