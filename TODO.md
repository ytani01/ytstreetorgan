...
**番号は `TODO-057` から。** かつては A・B・C … の 1 文字だったが、
...
- [ ] メモリの「完了へ移すときはコミットハッシュを添える」を削除する
- [ ] SessionStart フックから `## 着手順` の読み飛ばしを削除する
- [ ] `CLAUDE.md` の `archives/` の記述（グローバルと重複）を削除する

`~/.claude/CLAUDE.md`（全プロジェクト共通）と、このリポジトリの実態を
突き合わせたら 4 点ずれていた。**今後はグローバルに寄せる。過去のものは
直さない**（2026-08-07 に決めた）。

1. コミットハッシュを添える決まりはグローバルに無く、56 件中 4 件しか
   守られていない
2. 「立てる」「済んだ」の 2 コミットに分ける決まりが TODO-038 以降は
   守られていない
3. 完了コミットの `（TODO-NNN）` の書式がばらついている
4. `## 着手順` という見出しを知っているのは SessionStart フックだけで、
   どの文書にも無い

2 と 3 は今後の書き方の話なのでファイルの変更は無い。
