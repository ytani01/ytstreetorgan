...
更新: 2026-08-19（TODO-104 を追加）

**残っている項目: TODO-104。** 新しく足すときは、 **完了済み** の上に節を作る（完了したら「完了済み」へ移す）。**番号は `TODO-105` から。**
...
（Sonnet / effort low。サブエージェントは編成しない）

- [ ] `~/.claude/CLAUDE.md` のアーカイブの骨格の記述に、実施したモデルと
      effort を書く決まりを足す

項目を立てるときにモデルと effort を書く決まりは既にあるが、アーカイブへ
移す時点で消えている（TODO-103 のファイルにも残っていない）。あとから
「この規模ならどれで足りたか」を振り返る材料が無い。

決めること:

- 書式。既存の骨格（きっかけ / やったこと / テスト）を崩さないよう、
  冒頭に `実施: Opus 5 / effort high（サブエージェントなし）` のような
  1 行を置く案
- 残すのは**実際に実施したほう**（立てたときの見込みと食い違うことがある）
- サブエージェントを編成した項目は、`archives/agents/TODO-NNN/README.md`
  に分担とモデルを書く決まりが既にあるので、そちらへの参照でよいか

直すのはグローバルの `~/.claude/CLAUDE.md` で、このリポジトリのファイル
ではない（TODO-057 と同じ扱い）。既存の 103 件には遡らない（実施した
モデルの記録が無く、書けない）。
