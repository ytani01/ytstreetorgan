...
更新: 2026-08-19（TODO-101 を立てた）

**残っている項目: TODO-101。** 新しく足すときは、 **完了済み** の上に節を作る（完了したら「完了済み」へ移す）。**番号は `TODO-102` から。**
...
- [ ] 対象の洗い出し（`survey` に投げる）
- [ ] 文書（`*.md`）の直し
- [ ] コード内のコメント・docstring の直し（`src/` と `tests/`）
- [ ] `pytest` / `ruff` / `mypy`

ユーザー全体の `CLAUDE.md` に「日本語の書き方」の節を足した（2026-08-19）。
不自然な直訳、要らない主語、専門用語の無理な和訳を直す。
**`archives/` 以下は対象外**（決着した記録なので直さない）。
用語そのものの決めごとは `CLAUDE.md`「画面に出す用語」が優先で、
そこで決めた語（「音名」「試聴」など）は変えない。

対象: `docs/`、各 `CLAUDE.md`、`README.md`、`src/` と `tests/` の
コメント・docstring、**画面に出る文言**。

モデル: Opus / effort: medium（自然さの判断が要るため）。
サブエージェントは編成せず、洗い出しだけ `survey` に投げる。

### 洗い出しの結果（`survey`）

`docs/`・各 `CLAUDE.md`・`README.md`・`DOCTOR.md`・JS / HTML のコメントは
**該当なし**。直すのは下の 15 行だけ。

**`src/ytstreetorgan/rollbook.py`（docstring の 1 行目）**

| 行 | 現在 | 案 |
|---|---|---|
| 44 | MIDI ノート番号からトラック番号（インデックス）を取得する。 | MIDI ノート番号に対応するトラック番号を返す。 |
| 128 | 矩形描画用のSVGパス文字列を生成する。 | 矩形を描く SVG のパス文字列を返す。 |
| 234 | HoleInfoのインスタンスを初期化する。 | 音符 1 つぶんの穴の位置と長さを求める。 |
| 271 | オブジェクトの文字列表現を取得する。 | デバッグ用に、穴の諸元を 1 行にまとめる。 |
| 287 | 穴描画用のSVGパス文字列を生成する。 | 穴を描く SVG のパス文字列を返す。 |
| 327 | RollBookインスタンスを初期化する。 | 機種の設定を読み、移調量を決めておく。 |
| 499 | ロールブック全体を描画するSVGドキュメント文字列を生成する。 | ブック全体の SVG を組み立てて返す。 |

**`src/ytstreetorgan/rollbook.py`（`Returns:` の説明）**

| 行 | 現在 | 案 |
|---|---|---|
| 142, 295 | 生成されたSVGパス要素の文字列。 | SVG のパス要素の文字列。 |
| 508, 605, 624 | 生成されたSVG形式のテキスト文字列。 | SVG のテキスト。 |

**画面の文言**

- `webroot/templates/storgan.html:51` —
  「機種の音階に無い音は穴が開きませんが、キーを変えることで出来る限り
  音源に近いメロディーを再現できるよう最適化することができます。」
  → 「機種の音階に無い音は穴が開きませんが、キーを変えて、なるべく
  音源に近いメロディーになるよう最適化します。」（「できる」が二重で冗長）

**テスト**

- `tests/test_conf.py:128` — 「`FileNotFoundError` が送出される」
  → 「`FileNotFoundError` になる」。**例外名は残す**（「例外になる」と
  ぼかすより、名前があるほうが分かりやすい）

`Args:` の `(str, optional)` 形式や「デフォルトは X」は言い回しの
問題ではないので触らない。`CLAUDE.md`「画面に出す用語」で決めた語も
変えない。
