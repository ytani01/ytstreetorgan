...
**残っている項目: TODO-090、TODO-091、TODO-092。** これまでに 89 件を決着させた。
...
`CLAUDE.md`「画面に出す用語」では、**「試聴」はブラウザで鳴らして確かめる
こと**を指す（TODO-063）。CLI の `play` に使うと意味がずれる。
「鳴らす」「再生する」に直す（TODO-089 で見つけた）。

**TODO-089 と TODO-090 のあとに着手する**（リンク先ができてから）。
`docs/` へのリンクは TODO-089 で足したので、残っているのは体裁を揃える
ぶんだけ。
