...
更新: 2026-08-19（TODO-100 を追加）

- 新しく足すときは、 **完了済み** の上に節を作る（完了したら「完了済み」へ移す）。**番号は `TODO-101` から。**
...
- [ ] ただし `tests/browser/conftest.py:1` はクラスではない（fixture を
      集めたモジュール）。「ブラウザテスト用の fixture」のようにする
- [ ] 「持ち帰り」「持ち帰る」→「ダウンロード」。**文ごと書き直してよい**
      （意味は変えない）
- [ ] `storgan.html:281` の `title` を
      「移調した MIDI をダウンロード（ロールブックではありません）」にする
- [ ] `CLAUDE.md` の「画面に出す用語」の表に「ダウンロード」の行を足す
- [ ] `archives/todo/` の TODO-095 / 096 / 099 も直す。**TODO-096 は
      見出し・ファイル名・`TODO.md` の目次を揃える**（`git mv`）

範囲外: 上の 3 件以外の `archives/`、`TODO.md` の TODO-050 / TODO-075 の
目次行（アーカイブのファイル名をそのまま出しているため）。

モデル / effort: Sonnet 5 / low（サブエージェントは編成しない）
...
- [**TODO-096.** 持ち帰り系のヘッダ組み立てが 3 か所に写してある](archives/todo/TODO-096.%20持ち帰り系のヘッダ組み立てが%203%20か所に写してある.md)
