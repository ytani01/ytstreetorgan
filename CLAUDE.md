...
`midi_to_note_name()` が MIDI ノート番号との変換を受け持つ。穴の位置を
決めるのは音名だけで、`'base_note'` との差（半音単位のオフセット）は
`note_offsets()` がその場で導出する（設定には持たない）。
かつては `'note name'` と `'note offset'` の 2 本の並行配列、その後は
`{'name': str, 'offset': int}` の辞書のリストだったが、`'offset'` を
設定に持たせず導出する今の形に変えた（TODO-013、TODO-064）。
...
| 基準の音 | 半音単位のオフセット（`'base_note'` との差）を数える起点。値は MIDI ノート番号 | `'base_note'` |
...
- **「ノート」は必ず「MIDI ノート番号」と書く**（音名と紛れるため）。
  裸の「ノート」は使わない
- **半音単位のオフセットは画面に出さない**（TODO-064）。設定エディタは
  音名のドロップダウン 1 つで入力し、選択肢には参考として MIDI ノート番号を
  丸括弧で添える（`F4 (65)`）。オフセットは `note_offsets()` が内部で
  導出するだけの値で、画面や文書に出すときは「半音単位のオフセット」と
  単位まで書く
